 ...
 * @author      Paul Massendari <paul@massendari.com>, Modified by Jybrid
...
class seoPlugin extends Plugin
{
	 ...
	 * @return string
	 */
	private function removeBadWords(string $text): string
	{
		$bad = ['Table of contents:'];

		$replace = [''];

		return trim(str_replace($bad, $replace, $text));
	}
	 ...
	 * @return string
	 */
	private function cleanupExtras(string $text): string
	{
		$search  = ["\n", '"', '<p', '</p>', '. .', '  ', '  '];
		$replace = ['', '', '', '', '.', ' ', ' '];
	...
	public function onPageInitialized()
	{
		...
		if (property_exists($page->header(), 'twitterenable') && 'true' === $page->header()->twitterenable)
		{
			...
			else if (!empty($page->media()->images()))
			{
		...
		if (property_exists($page->header(), 'facebookenable') && 'true' === $page->header()->facebookenable)
		{
		...
		{
			if ($page->header()->restaurantenabled && $this->config['plugins']['seo']['restaurant'])
			{
	...
	public function onBlueprintCreated(Event $event)
	{
		...
		else
		{
			...
			{
				$blueprint->extend((new Blueprints(__DIR__ . '/blueprints/'))->get($this->name), true);
			}
		}
	}
